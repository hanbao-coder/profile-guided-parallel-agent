...
开始，保留模型读取记录、修改、项目测试、固定工作负载输出和端到端计时。B1 普通 Agent 和
B2 完整方法各独立运行 12 次，即每个项目 3 次；B0 是每次试验现场重跑的串行基线。
...
- B1 普通 Agent：2 次错误修改，1 次没有形成方案；
- A1 只加语义约束：2 次错误修改，1 次没有形成方案；
- B2 完整方法：3 次都发现候选不合格并安全回退。
...
- 原始文件哈希清单：`docs/data/research-evidence-manifest.json`；
