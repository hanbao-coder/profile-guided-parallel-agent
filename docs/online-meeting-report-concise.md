...
![真实 Ray 单节点主实验的时间减少比例和结果比例](assets/online_meeting_expanded_20260804/主实验补充分析.png)

为了看清优化具体改善了哪些任务，我又把 M2 与 M1 的运行时间逐项比较。大量小任务的运行时间减少了 91.8%，大数据传输减少了 59.9%，两两距离计算减少了 28.9%。这些正是固定拆分容易产生额外开销的任务。

素数统计和随机模拟中，M1 本身已经比较合适，所以 M2 的改进较小。这个现象也提醒我，优化方法不应该要求每一类任务都出现很大的提升；当朴素方案已经合适时，保持结果稳定同样重要。

右图统计的是 8 类任务、3 次独立实验得到的 24 组运行组合。M1 有 37.5% 的组合比串行慢 5% 以上，M2 没有出现这类退化；但 M2 获得加速的比例是 91.7%，而不是 100%。Large Payload 和 Pairwise Distance 的加速仍然较小，报告中保留了这些结果。
...
消融实验的做法是：保留大部分设置，只去掉或固定其中一个环节，然后观察结果怎样变化。它不能代替主实验，但可以帮助判断改进来自哪一部分。

| 对比内容 | 改变的部分 | 我想确认的问题 |
|---|---|---|
| Agent 反馈方式 | 是否查看性能并允许回退 | 只检查正确性是否足够 |
| 配置搜索 | 固定配置、小样本判断、完整搜索 | 搜索 Worker 和 Chunk 是否有用 |
| 任务融合 | 不融合、固定融合、判断后融合 | 哪些任务适合放在一起执行 |
| DAG 调度 | FIFO、关键路径优先 | 提交顺序是否影响总完成时间 |

### 6.1 Agent 反馈方式

![Agent 反馈方式消融：一次生成、正确性反馈和性能反馈](assets/online_meeting_expanded_20260804/Agent反馈消融.png)

这组实验只选用 Tiny Tasks，比较三种做法：一次生成后直接使用、只根据正确性修复、同时查看性能。前两种方法生成的代码虽然正确，但加速比都不到 0.5，也就是比串行慢了一倍以上。

加入性能反馈后，候选 Ray 代码本身仍然没有变快，实测加速比只有 0.467。系统发现并行不划算后回退到串行，有效加速比恢复到 1.0。这说明性能反馈的作用不一定是把每段代码都改得更快，有时更重要的是不保留明显更慢的版本。

这是一组单任务受控实验，只用于说明反馈模块的作用，不作为 8 类 Ray 主实验的总体结论。

### 6.2 配置搜索

![配置搜索消融：固定配置、小样本判断和完整搜索](assets/online_meeting_expanded_20260804/配置搜索消融.png)

早期版本固定使用 4 个 Worker 和 4 个 Chunk，整体平均加速比只有 0.926，70.8% 的运行组合出现性能退化。只做小样本收益判断后，系统会拒绝明显不合适的并行方案，因此总体结果回到 1.0，退化率降到 0%，但没有进一步得到加速。

完整配置搜索会在少量候选中选择 Worker 和 Chunk，平均加速比达到 1.149，同时保持 0% 退化。它说明“是否并行”和“怎样并行”是两个不同问题：收益判断负责避开明显错误的选择，配置搜索负责在值得并行时继续寻找更合适的参数。

这组数据来自项目早期的本地多进程实验，运行后端与正式 Ray 实验不同。因此这里仅验证配置搜索的方向，不把数值与 Ray 主实验合并。

[PAGEBREAK]

### 6.3 任务融合

![任务融合消融：不融合、固定融合和判断后融合](assets/online_meeting_expanded_20260804/任务融合消融.png)

任务融合是把前后两个小步骤放到同一个 Worker 内完成，减少中间结果的传递。左图中，中间结果只交给一个后继任务，融合后运行时间从约 100.9 毫秒降到 4.3—4.6 毫秒，说明这种线性任务链适合融合。

右图中，同一份中间结果会被多个后继任务使用。如果强制融合，每个后继都要重复前面的计算，运行时间反而从 632.7 毫秒增加到 1160.1 毫秒。判断后融合会识别这种情况并保留原来的任务图，运行时间为 565.2 毫秒。

因此系统没有采用“一律合并”的规则，而是先看中间结果有几个使用者，再决定是否融合。这组实验是本地受控实验，用来检查融合规则，不作为真实 Ray 多机性能结果。

[PAGEBREAK]

### 6.4 DAG 调度顺序

![FIFO 与关键路径优先的任务图调度对比](assets/online_meeting_expanded_20260804/DAG调度对比.png)

DAG 可以理解为一张带有先后依赖的任务流程图。FIFO 按任务进入队列的先后顺序提交，规则简单；关键路径优先会先处理那些一旦变慢就容易拖住整张任务图的任务。

在两个固定任务图模型中，关键路径优先把总完成时间从 2.18 降到 1.85、从 0.98 降到 0.78，同时降低了 Worker 空闲比例。这说明当任务之间有多层依赖、不同分支耗时差异较大时，提交顺序可能影响最终完成时间。

这部分目前只完成了确定性模型验证，还没有接入真实 Ray 多机执行。因此它用于说明调度模块的设计思路，不能当作真实分布式性能结论。
...
补充消融实验进一步说明了各部分分别解决什么问题：性能反馈避免保留正确但更慢的候选，配置搜索改善固定参数，任务融合需要区分单后继和多后继情况，关键路径优先在任务图模型中减少了等待时间。

这些结果不能证明任意 Python 程序都能被自动高效并行化，也不能替代真实多机实验。它们目前能够说明：在所覆盖的数据并行任务中，真实运行反馈和任务粒度选择比单纯生成并行代码更重要。任务融合和 DAG 调度目前仍是补充验证，下一步需要放到真实 Ray 环境中继续测试。
